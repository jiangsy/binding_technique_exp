...
Fixpoint num_tvars (Δ : ctx) :=
  match Δ with
  | nil => 0
  | entry_tvar _ :: Δ => 1 + num_tvars Δ
  | _ :: Δ => num_tvars Δ
  end.

Corollary ctx_tvar_renaming_tvars Δ1 Δ2 A: 
  ctx_tvar_rename Δ1 (Δ2 ++ (entry_tvar A) :: Δ1) (fun X : nat => 1 + (num_tvars Δ2 + X)).
Proof.
  ...
  - destruct a; eauto using lookup_tvar.
    replace (⟨fun X0 : nat => S (S (num_tvars Δ2 + X0))⟩ B) with
    (B ⟨fun X : nat => 1 + (num_tvars Δ2 + X)⟩ ⟨ ↑⟩ ) by (asimpl; eauto).
    eauto using lookup_tvar.
Qed.

Lemma lookup_tvar_rebounding : forall Δ1 Δ2 B,
  lookup_tvar (num_tvars Δ2) (Δ2 ++ (entry_tvar B) :: Δ1) (B ⟨fun X => 1 + (num_tvars Δ2 + X) ⟩) /\
  (forall X A B', X <> num_tvars Δ2 -> 
    lookup_tvar X (Δ2 ++ (entry_tvar B) :: Δ1) A ->
    lookup_tvar X (Δ2 ++ (entry_tvar B') :: Δ1) A).
Proof.
  split. 
  - induction Δ2; simpl; asimpl; try hauto ctrs:lookup_tvar.
    + destruct a; try hauto ctrs:lookup_tvar. 
      replace (ren_typ (Init.Nat.add (S (num_tvars Δ2)) >> S) B) with
      (B ⟨fun X : nat => 1 + (num_tvars Δ2 + X)⟩ ⟨↑⟩ ) by (asimpl; auto).
  ...
  - intros. move : C H1. induction H0; intros; try hauto ctrs:subtyping.
    ...
    + inversion H1; subst.
      * eapply sub_top. simpl in *. split.
        ...
        -- eapply wf_typ_renaming_tvar' with (ξ:=id) (A:=A2) (A':=A2) (Δ:=(entry_tvar B1)::Δ);
      ...
      * eapply sub_all; eauto.
        ...
        -- edestruct (IH (typ_size B1)). eauto. hfcrush. eauto. 
           ...
           replace ((entry_tvar B0) :: Δ) with (nil ++ (entry_tvar B0) :: Δ) by auto.
           eapply H2; eauto.
  - intros Htrans. intros. move : B' H1. dependent induction H0; intros; auto.
    ...
    + eapply sub_tvar_refl; eauto.
      eapply wf_typ_renaming_tvar' with (ξ:=id) (A:=typ_var X); 
        hauto use:ctx_tvar_rename_weak_rebounding simp+:asimpl.
    + edestruct (Nat.eq_dec X (num_tvars Δ2)).
      * subst. 
        eapply sub_tvar_bound with (A:=B' ⟨fun X => 1 + (num_tvars Δ2 + X) ⟩).
        hauto use:lookup_tvar_rebounding. eapply Htrans with (B:=B ⟨fun X => 1 + (num_tvars Δ2 + X) ⟩).
        -- hauto use:typ_size_renaming_eq.
        -- eapply sub_renaming_tvar; eauto.
           eapply ctx_tvar_renaming_tvars.
        -- erewrite (lookup_tvar_det _ _ (⟨fun X : nat => 1 + (num_tvars Δ2 + X)⟩ B)); eauto.
    ...
    + eapply sub_all; try hauto ctrs:subtyping.
      * eapply wf_typ_renaming_tvar' with (ξ:=id); 
        hauto use:ctx_tvar_rename_weak_rebounding simp+:asimpl.
      * replace ((entry_tvar B1) :: Δ2 ++ (entry_tvar B') :: Δ1) with ((entry_tvar B1 :: Δ2) ++ (entry_tvar B') :: Δ1) by auto.
